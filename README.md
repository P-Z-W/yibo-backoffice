...
- 报销管理：快速逐行录入、主体与税号、Excel 批量分组导入、发票图片/PDF 识别、识别结果人工修正、主管审批、可选财务审批、任意状态批量导出及导出批次追踪。
...
一期发票识别使用百度智能云增值税发票识别接口。未配置密钥时，发票仍可正常上传，并可直接人工录入或修正主体、税号和金额。需要启用自动识别时，在 `backend/.env` 增加：

```dotenv
INVOICE_OCR_PROVIDER=baidu
BAIDU_OCR_API_KEY=你的_API_Key
BAIDU_OCR_SECRET_KEY=你的_Secret_Key
INVOICE_OCR_TIMEOUT_SECONDS=30
```

识别返回的原始值会单独保留；用户确认后的主体、税号、金额作为最终业务值，人工修改不会覆盖原始识别记录。
